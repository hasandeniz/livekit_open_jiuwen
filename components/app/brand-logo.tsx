/**
 * Huawei brand logo, inlined so the wordmark can follow the active design.
 * The wordmark uses `currentColor` (set via the `--ink` token by the parent),
 * so it is dark on the light design and light on the dark designs; the flower
 * keeps its brand red on every theme.
 */
export function BrandLogo({ title, className }: { title?: string; className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 1275.5 287"
      className={className}
      role="img"
      aria-label={title}
    >
