...
export function ElevenLabsTest({ configured }: { configured: boolean }) {
  return (
    <ConversationProvider>
      <TestSession configured={configured} />
    </ConversationProvider>
  );
}

function TestSession({ configured }: { configured: boolean }) {
      ...
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="mb-2 font-mono text-xs tracking-widest text-(--aqua) uppercase">
            Voice lab · Development
          </p>
          <h1 className="text-3xl font-semibold tracking-tight">ElevenLabs conversation test</h1>
      ...
      {!configured && (
        ...
        >
          ...
          <p className="mt-2 text-sm">
            ...
            instructions are in <code>frontend/ELEVENLABS_TEST.md</code>.
          </p>
        </div>
      )}
